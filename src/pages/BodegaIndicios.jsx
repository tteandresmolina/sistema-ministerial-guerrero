  ...
  if (!esNarco) {
    return (
      <div style={{ ...cardStyle, textAlign: 'center', padding: 40 }}>
        ...
        <div style={{ color: '#9ca3af', fontSize: 16 }}>Acceso restringido a la Fiscalía Especializada en Delitos de Narcomenudeo, Coordinación Regional y Director General.</div>
      </div>
    );
  }
    ...
    <div>
      ...
      {indicios.length === 0 ? (
        <div style={{ ...cardStyle, textAlign: 'center', padding: 40, color: '#9ca3af' }}>
          ...
          <div style={{ fontSize: 16 }}>No hay indicios registrados con los filtros actuales</div>
        </div>
      ) : (
        indicios.map(ind => (
          <div key={ind.id} style={{ ...cardStyle, cursor: 'pointer', borderLeft: `4px solid ${getEstatusColor(ind.estatus)}`, transition: 'box-shadow 0.2s' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
              <div style={{ flex: 1, minWidth: 220 }}>
                ...
                <div style={{ fontSize: 15, color: '#374151', fontWeight: 600 }}>{ind.descripcion}</div>
              ...
              <div style={{ textAlign: 'right', minWidth: 120 }}>
                <div style={{ fontSize: 13, color: '#9ca3af' }}>{formatFecha(ind.fecha_entrada)}</div>
                {ind.folio_cadena_custodia && <div style={{ fontSize: 12, color: '#6b7280', fontFamily: 'monospace', marginTop: 4 }}>CC: {ind.folio_cadena_custodia}</div>}
                {ind.ubicacion_bodega && <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>{ind.ubicacion_bodega}</div>}
              </div>
            </div>
          </div>
        ))
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <div style={{ gridColumn: '1 / -1', borderTop: '2px solid #e8ecf1', paddingTop: 14 }}>
            <div style={{ fontWeight: 700, color: COLORS.gold, fontSize: 15, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Entrega — Recepción</div>
          </div>
        ...
        <div style={{ borderTop: '2px solid #e8ecf1', paddingTop: 14, marginTop: 14 }}>
          ...
          <div style={{ border: '2px dashed #c7cfe0', borderRadius: 10, padding: 20, textAlign: 'center', cursor: 'pointer', background: COLORS.bg }}
            ...
            <p style={{ fontSize: 14, color: '#999', margin: '6px 0 0 0' }}>Clic para tomar o seleccionar foto</p>
      ...
      <div>
        ...
        <div style={cardStyle}>
          <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {ind.observaciones_entrada && <div style={{ fontSize: 14, color: '#6b7280', marginTop: 6, fontStyle: 'italic' }}>Obs: {ind.observaciones_entrada}</div>}
            </div>
          </div>
            ...
            {!showMovimiento ? (
              ...
              <div style={{ background: '#fffbeb', borderRadius: 8, padding: 14, borderLeft: `4px solid ${COLORS.gold}` }}>
                ...
                {formMov.tipo_movimiento === 'salida_destruccion' && (
                  <div style={{ background: '#fef2f2', border: '1px solid #ef444440', borderRadius: 8, padding: 10, marginTop: 10, fontSize: 14, color: '#7f1d1d', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={cardStyle}>
          ...
          {movimientos.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 20, color: '#9ca3af', fontSize: 15 }}>Sin movimientos — el indicio permanece en resguardo original.</div>
          ) : (
                  ...
                  <div key={m.id} style={{ position: 'relative', marginBottom: 10 }}>
                    ...
                    <div style={{ ...cardStyle, marginBottom: 0, padding: 12, borderLeft: `3px solid ${c}` }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 15, color: COLORS.primary }}>{getMovLabel(m.tipo_movimiento)}</div>
                          {m.destino && <div style={{ fontSize: 14, color: '#374151', marginTop: 2 }}>Destino: {m.destino}</div>}
                          <div style={{ fontSize: 14, color: '#6b7280', marginTop: 2 }}>Entrega: {m.nombre_entrega} → Recibe: {m.nombre_recibe}</div>
                          {m.folio_cadena_custodia && <div style={{ fontSize: 13, color: '#6b7280', marginTop: 2, fontFamily: 'monospace' }}>CC: {m.folio_cadena_custodia}</div>}
                          {m.observaciones && <div style={{ fontSize: 14, color: '#374151', marginTop: 4, fontStyle: 'italic' }}>"{m.observaciones}"</div>}
                        </div>
